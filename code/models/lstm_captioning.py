...
import torch
import sys
sys.path.append('/mnt/hdd4/leiyu/ssl/vatex_irv2/models/')
import torch
...
# dir = '/mnt/hdd1/leiyu/all_dataset/HGR_T2V/MSVD/results/RET.released/mlmatch/vis.resnet152.pth.txt.bigru.16role.gcn.1L.attn.1024.loss.bi.af.embed.4.glove.init/model/epoch.18.th'
# dir = '/mnt/hdd1/leiyu/all_dataset/HGR_T2V/MSVD/results/RET.released/mlmatch/vis.i3d+irv2.pth.txt.bigru.16role.gcn.1L.attn.1024.loss.bi.af.embed.4.glove.init/model/epoch.10.th'
dir = '/mnt/hdd4/leiyu/all_dataset/HGR_T2V/VATEX/results/RET.released/mlmatch/vis.c3d+irv2.pth.txt.bigru.16role.gcn.1L.attn.1024.loss.bi.af.embed.4.glove.init/model/epoch.43.th' #9
...
class LSTMCaptionModel(_CaptioningModel):
    def __init__(self, args, seq_len, vocab_size, bos_idx, h2_first_lstm=False, vid_second_lstm=False):
        ...
        self.feature_appearence = 1536 #1536
        self.feature_size_motion = 1024 #1024
        self.ss_prob = 0.0
        ...
        #======== hierarchical ========#   
        self.ft_embed_1 = nn.Linear(self.feature_size_motion, self.feature_size, bias=True) # com
        ...
        if self.vid_second_lstm:
            ...
            self.lstm_cell_2 = nn.LSTMCell(self.rnn_size + self.rnn_size + self.rnn_size + self.rnn_size, self.rnn_size)
        else:
    ...
    def init_hidden(self, feature): 
        ...
        # feat = torch.cat((feat,motion),2) #com
        feat_mask = (torch.sum(feat, -1, keepdim=True) != 0).float()
        feat_mean = torch.sum(feat, 1) / torch.sum(feat_mask, 1)  # (1,m,visual_size)
        state1 = (self.img_embed_h_1(feat_mean).to(device), self.img_embed_c_1(feat_mean).to(device))
        state2 = (self.img_embed_h_2(feat_mean).to(device), self.img_embed_c_2(feat_mean).to(device))
        state3 = (self.img_embed_h_3(feat_mean).to(device), self.img_embed_c_3(feat_mean).to(device))
    ...
    def step(self, t, state, prev_outputs, feats, seqs, previous_cells_, mode='teacher_forcing'):
        ...
        feat, motion = feats
        # feat = self.dropout_1(torch.cat((feat,motion),2)) #com
        b_s = feat.size(0)
        ...
        if self.vid_second_lstm:
            ...
            input_2 = torch.cat([state_1[0], c_a_t, c_o_t, func_feats], 1)
        else:
