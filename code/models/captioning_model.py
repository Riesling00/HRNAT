...
class CaptioningModel(Module):
    ...
    def forward(self, args, batch_data):
        ...
        state, hidden = self.init_hidden((images,motions)) #com

        out = None
        loss_match = 0
        loss_re = 0.
        loss_pos = 0.
        outputs = torch.zeros(args.batch_size, seq_len, args.vocab_size).to(device)
        decoder_hiddens = torch.zeros(state[1][0].size(0), seq_len, state[1][0].size(1)).to(device)
        module_weights = []
        self.previous_cells = []
    ...
    def beam_search(self, visual, max_len: int, eos_idx: int,
        ...
        # b_s = visual.size(0)
        state, _ = self.init_hidden(visual) #com
